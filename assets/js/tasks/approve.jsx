   ...
   componentDidMount(){
    ...
    render(){
     ...
     var tab = []
        ...
        const listItems = this.state.list_tasks_new.map((d)=> <tr><td>{d.job_code}</td><td>{d.hours}</td><td>{d.manager}</td><td><button onClick = {() => update_task(d)}>Approve</button></td></tr>)
        tab.push(<table className="table table-dark"><thead><tr><th>Job Code</th><th>Hours</th><th>User ID</th><th>Approve/Reject</th></tr></thead><tbody>{listItems}</tbody></table>)
